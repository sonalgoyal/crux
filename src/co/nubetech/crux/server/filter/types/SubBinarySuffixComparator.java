...
public class SubBinarySuffixComparator extends SubBinaryComparator {

	
	public SubBinarySuffixComparator() {
		
	}

	public SubBinarySuffixComparator(byte[] value, int offset, int length) {
		...
		if (value != null) {
			...
			if (valueToCompareWith.length <= valueToCompare.length) {
				result = Bytes.compareTo(valueToCompareWith, 0,
						valueToCompareWith.length, valueToCompare,
						valueToCompare.length - valueToCompareWith.length,
